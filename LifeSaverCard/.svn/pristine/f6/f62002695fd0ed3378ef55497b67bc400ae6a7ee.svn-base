...
public class WXEntryActivity extends Activity implements IWXAPIEventHandler {
    ...
    SubscriberOnNextListener<WxInfoBean> onNextListener = new SubscriberOnNextListener<WxInfoBean>() {
        ...
        public void onNext(WxInfoBean wxInfoBean) {
            ...
            session_id = wxInfoBean.getSession_id();
            id = wxInfoBean.getId();
            headImg = wxInfoBean.getHeadImg();
            if (status != -1) {
                intent.setClass(WXEntryActivity.this, MainActivity.class);
                startActivity(intent);
                preference.putString(LifeSaverPreference.SESSION_ID, session_id);
                preference.putString(LifeSaverPreference.ID, String.valueOf(id));
